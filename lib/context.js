...
export const SchedulesProvider = ({ children }) => {
    ...
    const deleteSchedule = (scheduleId) => {
        if (schedules.length <= 1) return false; // Don't delete the last schedule
            ...
            prev.map((schedule) => {
                if (schedule.id === scheduleId) {
                    ...
                    const newSelectedCourses =
                        index > -1
                            ? schedule.selectedCourses.filter(
                                  (c) => c.id !== course.id
                              )
                            : [...schedule.selectedCourses, course];
    ...
    const toggleSelected = (course) => {
        if (activeScheduleId) {
            toggleSelectedInSchedule(activeScheduleId, course);
        }
    };
...
// Combined hook that uses schedules when available, falls back to legacy
