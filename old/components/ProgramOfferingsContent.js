...
export default function ProgramOfferingsContent({
  ...
  // Handle prefill data from external integration
  useEffect(() => {
    if (prefillData && !isLoadingPrefill) {
      const { courses } = prefillData;

      // Set the custom courses directly
      setCustomCourses(courses);

      // Create a phantom program with the courses
      ...
      toast({
        ...
        description: `Your schedule has been loaded with ${courses.length} courses. You can now view and manage your courses.`,
      });

      // Clear the prefill data to prevent re-processing
      setPrefillData(null);
    }
  }, [prefillData, isLoadingPrefill]);
      ...
      let filtered = subjectOffering.filter((course) => {
        return curriculumCatNos.some((cat) => {
          ...
          if (/^nstp\s*/i.test(cat)) {
            const nstpNum = (cat.match(/^nstp\s*(\d+)/i) || [])[1]?.trim();
          ...
          if (/^philo\s*/i.test(cat)) {
            const philoNum = (cat.match(/^philo\s*(\d+)/i) || [])[1]?.trim();
            const match = course.catNo
              .toUpperCase()
          ...
          if (/^ie\s*/i.test(cat)) {
            const ieNum = (cat.match(/^ie\s*(\d+)/i) || [])[1]?.trim();
            const isInterdisciplinary = course.deptCode === "**IE**";
  ...
  const exportIPS = () => {
    ...
    link.href = url;
    link.download = `ateneo-ips-${new Date().toISOString().split("T")[0]}.json`;
          ...
          <div className="flex flex-col gap-2 ">
            ...
            >
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography variant="h6">IPS Selector</Typography>
              </AccordionSummary>
              <AccordionDetails>
