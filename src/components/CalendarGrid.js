...
export default function CalendarGrid({
  ...
  return (
    <div className="rounded-3xl border border-[color:var(--line)] bg-white/80 shadow-[0_20px_40px_-32px_rgba(15,23,42,0.6)]">
      <div
      ...
      >
        <div className="px-2 py-3 sm:px-3">Time</div>
        {days.map((day) => (
          ...
          {timeSlots.map((minutes) => {
            ...
            return (
              <Fragment key={`row-${minutes}`}>
                <div className="flex items-center border-b border-[color:var(--grid-line)] px-2 text-[0.6rem] text-[color:var(--muted)] sm:px-3 sm:text-[0.7rem]">
                  <span
                    className={
                      isHour
        ...
        >
          ...
          {days.map((day) => (
            <div key={day.label} className="relative">
              {availableSlotsByDay[day.index]?.map((slot) => {
                const top =
              ...
              {scheduledBlocksByDay[day.index]?.map((block) => {
                ...
                return (
                  <button
                    ...
                    className={`pointer-events-auto absolute left-1 right-1 flex flex-col items-center justify-center gap-1 rounded-xl border-2 px-1.5 py-1 text-center text-[0.55rem] font-semibold shadow-[0_10px_20px_-16px_rgba(15,23,42,0.6)] sm:left-2 sm:right-2 sm:px-2 sm:text-[0.65rem] ${block.section.colorClass} ${
                      dimScheduled && !isSelected ? "opacity-60" : ""
                    } ${isSelected ? "ring-2 ring-[color:var(--accent)]" : ""}`}
                    style={{ top, height }}
                    aria-label={`Scheduled ${block.section.catNo} ${block.section.section}`}
                  >
                    <span className="uppercase leading-tight tracking-normal sm:tracking-[0.1em]">
                      {block.section.catNo}
                    </span>
                    <span className="text-[0.55rem] font-medium leading-tight sm:text-[0.6rem]">
